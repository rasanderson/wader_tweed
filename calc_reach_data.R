...
flow_pct_diff <- data.frame(reach_no=1:23, pct=NA)
guaged_reach_no <- 6
for(i in 1:6){
...
# Repeat the process for the ungaugaed reaches
# Adjust the Qmm in accordance with values from earlier linear model
# Need to put all this in a function or simplify at some point
# As it is so slow will try as a function and parallise it (Linux-only)
for(reach_no in 8:11){
  print(paste0("Reach number:", reach_no))
  inca_sub <- inca23[inca23$reach_no == reach_no,]
  plot(inca_sub["reach_no"])
...
# Reaches 13 to 18 ungauged ####
# Estimate pct decline in flow upstream of Reach 19 to ungauged Reach 13
guaged_reach_no <- 19
for(i in 13:18){
  flow_pct_diff$pct[i] <- fitted(flow_lm)[i] / fitted(flow_lm)[guaged_reach_no]
}
# Ungauged Reaches 13 to 18
# Repeat the process for the ungaugaed reaches
# Adjust the Qmm in accordance with values from earlier linear model
for(reach_no in 13:18){
  print(paste0("Reach number:", reach_no))
  inca_sub <- inca23[inca23$reach_no == reach_no,]
  plot(inca_sub["reach_no"])
...
# Reaches 20 to 22 ungauged
# Estimate pct decline in flow upstream of Reach 19 to ungauged Reach 13
guaged_reach_no <- 23
for(i in 20:22){
  flow_pct_diff$pct[i] <- fitted(flow_lm)[i] / fitted(flow_lm)[guaged_reach_no]
}
# Ungauged Reaches 20 to 22
# Repeat the process for the ungaugaed reaches
# Adjust the Qmm in accordance with values from earlier linear model
for(reach_no in 20:22){
  print(paste0("Reach number:", reach_no))
  inca_sub <- inca23[inca23$reach_no == reach_no,]
  plot(inca_sub["reach_no"])
